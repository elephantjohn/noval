...
def sanitize_filename(title: str, chapter_number: int) -> str:
    # 仅保留汉字+数字+下划线
    name = f"第{chapter_number}章_" + title
    # 替换空格为下划线
    name = name.replace(" ", "_")
    # 只保留汉字、数字和下划线
    name = re.sub(r"[^\u4e00-\u9fff0-9_]", "", name)
    if not name:
...
def build_system_prompt(genre_label: str, world_brief: str) -> str:
    # 专业科幻作家，口语化规则，严格输出纯正文
    rules = (
        "你是一位专业的科幻小说作家。保持口语化与短句为主，信息密度与画面感高。\n"
        "仅输出纯正文，不要任何标题、结构标记、提纲、说明、注释或额外内容。\n"
        "避免模板腔；允许自然口语停顿。\n"
    )
...
def build_user_prompt(
    ...
    user_prompt = (
        ...
        f"[当前章节指令]\n{chapter_block}\n\n"
        f"[写作要求]\n"
        f"- 仅输出纯正文；不得出现任何标题/结构标记/提示语。\n"
        f"- 目标长度约4500字，允许自然浮动；保持时间线与事实一致，不剧透未来。\n"
        f"- 保留两句自然的‘废话’作为情绪缓冲或人物自语，必须与情节相关。\n"
    )
    return user_prompt
